...
GetLastNode:
	...
	// INSERT YOUR CODE HERE
	getlast:
	CBZ	X0, END		// Check if first node is NULL
	LDUR	X9, [X0, #8]	// Load next node
	CBZ	X9, END		//Check to see if next node is NULL
	ADDI	X0, X0, #8	// Update the pointer to be the next node 
	B	getlast		//Return to top of getlast and repeat the whole process 	

	END:
	ADDI	X1, XZR, X0	// Add the address of the last node to X1
	br lr 	
